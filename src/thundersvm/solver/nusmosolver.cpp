...
NuSMOSolver::calculate_rho(const SyncArray<float_type> &f_val, const SyncArray<int> &y, SyncArray<float_type> &alpha,
    ...
    for (int i = 0; i < alpha.size(); ++i) {
        if (y_data[i] > 0) {
            if (alpha_data[i] > 0 && alpha_data[i] < weights_data[i]) {
                n_free_p++;
                sum_free_p += f_val_data[i];
            }
            if (is_I_up(alpha_data[i], y_data[i], weights_data[i])) up_value_p = min(up_value_p, -f_val_data[i]);
            if (is_I_low(alpha_data[i], y_data[i], weights_data[i])) low_value_p = max(low_value_p, -f_val_data[i]);
        } else {
            if (alpha_data[i] > 0 && alpha_data[i] < weights_data[i]) {
                n_free_n++;
                sum_free_n += -f_val_data[i];
            }
            if (is_I_up(alpha_data[i], y_data[i], weights_data[i])) up_value_n = min(up_value_n, -f_val_data[i]);
            if (is_I_low(alpha_data[i], y_data[i], weights_data[i])) low_value_n = max(low_value_n, -f_val_data[i]);
        }
    }
...
NuSMOSolver::select_working_set(vector<int> &ws_indicator, const SyncArray<int> &f_idx2sort, const SyncArray<int> &y,
    ...
    while (n_selected < ws_size) {
        ...
        if (p_left_p < n_instances) {
            i = index[p_left_p];
            while (ws_indicator[i] == 1 || !(y_data[i] > 0 && is_I_up(alpha_data[i], y_data[i], weights_data[i]))) {
        ...
        if (p_right_p >= 0) {
            i = index[p_right_p];
            while (ws_indicator[i] == 1 || !(y_data[i] > 0 && is_I_low(alpha_data[i], y_data[i], weights_data[i]))) {
...
void NuSMOSolver::smo_kernel(const SyncArray<int> &y, SyncArray<float_type> &f_val, SyncArray<float_type> &alpha,
                             SyncArray<float_type> &alpha_diff, const SyncArray<int> &working_set, const SyncArray<float_type> &weights, float_type Cp,
                             float_type Cn,
