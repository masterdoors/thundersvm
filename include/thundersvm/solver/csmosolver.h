...
public:
    void solve(const KernelMatrix &k_mat, const SyncArray<int> &y, SyncArray<float_type> &alpha, float_type &rho,
               SyncArray<float_type> &f_val, float_type eps, SyncArray<float_type> &weights, float_type Cp, float_type Cn, int ws_size, int max_iter) const;
...
protected:
    ...
    smo_kernel(const SyncArray<int> &y, SyncArray<float_type> &f_val, SyncArray<float_type> &alpha,
               ...
               const SyncArray<int> &working_set, const SyncArray<float_type> &weights, float_type Cp, float_type Cn, const SyncArray<kernel_type> &k_mat_rows,
